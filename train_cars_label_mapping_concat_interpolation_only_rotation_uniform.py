...
def run(dataset, data_dir, result_dir, config_id, num_gpus, total_kimg, gamma, mirror_augment, metrics):
    train     = EasyDict(run_func_name='training.training_loop_interpolate.training_loop')          # Options for training loop.
    ...
    G_loss    = EasyDict(func_name='training.loss_interpolate.G_logistic_ns_pathreg_interpolate_w_only_rotation_concat')   # Options for generator loss.
    D_loss    = EasyDict(func_name='training.loss_interpolate.D_logistic_r1')                       # Options for discriminator loss.
    ...
    train.interpolation_dist = 'uniform'
    train.label_mixing = 'same'
    ...
    desc += '-%dgpu' % num_gpus
    desc += '-label-mapping-concat-interpolate-only-rotation-uniform'
