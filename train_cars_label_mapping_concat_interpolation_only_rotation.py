...
def run(dataset, data_dir, result_dir, config_id, num_gpus, total_kimg, gamma, mirror_augment, metrics):
    ...
    G_loss    = EasyDict(func_name='training.loss_label_mapping_concat_interpolate_only_rotation.G_logistic_ns_pathreg_interpolate')      # Options for generator loss.
    D_loss    = EasyDict(func_name='training.loss_label_mapping_concat_interpolate_only_rotation.D_logistic_r1')              # Options for discriminator loss.
    ...
    desc += '-%dgpu' % num_gpus
    desc += '-label-mapping-concat-interpolate-only-rotation'
