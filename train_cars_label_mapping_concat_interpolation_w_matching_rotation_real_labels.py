...
def run(dataset, data_dir, result_dir, config_id, num_gpus, total_kimg, gamma, mirror_augment, metrics):
    train     = EasyDict(run_func_name='training.training_loop_mirror_images_randomize_labels.training_loop') # Options for training loop.
    ...
    G_loss    = EasyDict(func_name='training.loss_label_mapping_concat_interpolate_w_matching_rotation_real_labels.G_logistic_ns_pathreg_interpolate')      # Options for generator loss.
    D_loss    = EasyDict(func_name='training.loss_label_mapping_concat_interpolate_w_matching_rotation_real_labels.D_logistic_r1')              # Options for discriminator loss.
