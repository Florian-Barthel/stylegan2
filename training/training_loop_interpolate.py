...
def process_reals(x, labels, lod, mirror_augment, drange_data, drange_net, keep_probability):
...
def interpolation_magnitude(interpolation_dist, sched):
    if interpolation_dist == 'uniform':
        return np.float32(np.random.uniform(size=[sched.minibatch_gpu]))
    if interpolation_dist == 'bathtub':
        return np.float32(sched.bathtub.rvs(size=[sched.minibatch_gpu]))


def different_labels_adjacent_rotation(training_set, sched):
    factor = 2
    label_left = training_set.get_random_labels_np(sched.minibatch_gpu)
    rotation_offset = 108
    num_rotations = 8
    rotations = label_left[:, rotation_offset:rotation_offset + num_rotations]
    rotation_index = np.argmax(rotations, axis=1)
    shifted_rotation_index = ((rotation_index + np.random.choice([-1, 1], size=[sched.minibatch_gpu])) % 8)
    num_uniques = 0
    label_right = training_set.get_random_labels_np(sched.minibatch_gpu * factor)
    while num_uniques < num_rotations:
        label_right = np.concatenate(
            [label_right, training_set.get_random_labels_np(sched.minibatch_gpu * factor)], axis=0)
        interpolate_rotations = label_right[:, rotation_offset:rotation_offset + num_rotations]
    ...
    label_right = label_right[unique_indices[shifted_rotation_index]]
    label_right[:, rotation_offset:rotation_offset + num_rotations] *= np.expand_dims(
        np.max(rotations, axis=1).astype(np.uint32), axis=1)

    return label_left, label_right


def same_labels_adjacent_rotation(training_set, sched):
    label_left = training_set.get_random_labels_np(sched.minibatch_gpu)
    rotation_offset = 108
    rotations = label_left[:, rotation_offset:rotation_offset + 8]
    rotation_index = np.argmax(rotations, axis=1)
    new_rotation_index = ((rotation_index + np.random.choice([-1, 1], size=[sched.minibatch_gpu])) % 8)
    new_rotation = np.zeros([sched.minibatch_gpu, 8], dtype=np.uint32)
    new_rotation[np.arange(sched.minibatch_gpu), new_rotation_index] = 1
    new_rotation = new_rotation * np.expand_dims(
        np.max(label_left[:, rotation_offset:rotation_offset + 8], axis=1).astype(np.uint32), axis=1)
    label_right = label_left.copy()
    label_right[:, rotation_offset:rotation_offset + 8] = new_rotation
...
def training_loop(
    ...
    keep_probability        = 1.0,      # Probability to keep each label
    interpolation_dist      = 'uniform',# random distribution for the interpolation
    label_mixing            = 'same',   # random distribution for the interpolation
    ...
    assert interpolation_dist in ['uniform', 'bathtub']
    assert label_mixing in ['same', 'different']
    ...
    with tf.name_scope('Inputs'), tf.device('/cpu:0'):
        ...
        label_left_var = tf.placeholder(tf.float32, name='label_left', shape=[None, 127])
        label_right_var = tf.placeholder(tf.float32, name='label_right', shape=[None, 127])
    ...
    for gpu in range(num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('DataFetch'):
                ...
                reals_write, labels_write = process_reals(reals_write, labels_write, lod_in, mirror_augment, training_set.dynamic_range, drange_net, keep_probability)
            ...
            with tf.control_dependencies(lod_assign_ops):
                with tf.name_scope('G_loss'):
                    G_loss, G_reg = dnnlib.util.call_func_by_name(
                        ...
                        interpolation_mag=interpolation_mag_var,
                        label_left=label_left_var,
                        label_right=label_right_var,
                        **G_loss_args)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        interpolation_mag = interpolation_magnitude(interpolation_dist, sched)
        if label_mixing == 'same':
            label_left, label_right = same_labels_adjacent_rotation(training_set, sched)
        if label_mixing == 'different':
            label_left, label_right = different_labels_adjacent_rotation(training_set, sched)
        ...
        feed_dict = {
            ...
            label_left_var: label_left,
            label_right_var: label_right,
