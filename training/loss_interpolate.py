...
def G_logistic_ns_pathreg_interpolate_w_only_rotation_concat(G, D, opt, training_set, minibatch_size, interpolation_mag, label_left, label_right, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0):
    _ = opt

    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    # latents_interpolate = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    dlatent = G.components.mapping_latent.get_output_for(latents, is_training=True)
    ...
    dlabel_left = G.components.mapping_label.get_output_for(label_left, is_training=True)
    dlabel_right = G.components.mapping_label.get_output_for(label_right, is_training=True)
    interpolation_mag_dlabel = tf.expand_dims(tf.expand_dims(interpolation_mag, axis=-1), axis=-1)
    mixed_dlabel = dlabel_left * interpolation_mag_dlabel + dlabel_right * (1 - interpolation_mag_dlabel)
    fake_dlatents_out = tf.concat([dlatent, mixed_dlabel], axis=-1)
    ...
    mixed_label = label_left * interpolation_mag_label + label_right * (1 - interpolation_mag_label)
