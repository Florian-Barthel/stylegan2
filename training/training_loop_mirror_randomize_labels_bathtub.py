...
def training_schedule(
    ...
    s.bathtub_scale = float(bathtub_init_scale / (10 ** (s.kimg / bathtub_reduce_f10_time)))
    ...
    for gpu in range(num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('DataFetch'):
                ...
                bathtub = BathtubDistribution(a=0.0, b=1.0, name='bathtub', scale=sched.bathtub_scale)
                interpolation_mag = bathtub.rvs(size=[sched.minibatch_gpu])
            ...
            with tf.control_dependencies(lod_assign_ops):
                with tf.name_scope('G_loss'):
                    G_loss, G_reg = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, training_set=training_set, minibatch_size=minibatch_gpu_in, interpolation_mag=interpolation_mag, **G_loss_args)
