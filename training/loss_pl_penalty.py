...
def G_logistic_ns_pathreg_pl_penalty(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0):
    _ = opt
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    labels = training_set.get_random_labels_tf(minibatch_size)
    fake_images_out, fake_dlatents_out = G.get_output_for(latents, labels, is_training=True, return_dlatents=True)
...
def G_logistic_ns_pathreg_interpolate(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01,
    ...
    # 1. get dlatents for the batch by running G.mapping (Shape: [4, 16, 512])
    dlatents = G.components.mapping.get_output_for(latents, labels, is_training=True)

    # define an interpolation magnitude
    # interpolation_mag = 0.5
    interpolation_mag = tf.clip_by_value(tf.random.normal([1], 0.5, 0.15), 0, 1)

    # 2. interpolate between interpolate between dlatents[0] and dlatents[1]  (Shape: [1, 16, 512])
    dlatent_interpolate = tf.expand_dims(tflib.lerp(dlatents[0], dlatents[1], interpolation_mag), axis=0)

    # 3. replace dlatents[0] with interpolated dlatent (Shape: [4, 16, 512])
    dlatents_replaced = tf.concat([dlatent_interpolate, dlatents[1:]], axis=0)

    # 4. set fake_dlatents_out
    fake_dlatents_out = dlatents_replaced
    ...
    # 7. run G.synthesis with the new dlatents and generate the fake images
