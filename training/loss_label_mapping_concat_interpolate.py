...
def G_logistic_ns_pathreg_interpolate(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01,
    ...
    interpolation_mag = tf.random.uniform([], minval=0, maxval=1)

    # 3. interpolate between interpolate between dlatents[0] and dlatents[1]  (Shape: [1, 16, 512])
    dlatent_interpolate = tf.expand_dims(dlatents[0] * interpolation_mag + dlatent_interpolate[0] * (1 -interpolation_mag), axis=0)
    ...
    # wx1 = tf.where(labels[0] > label_interpolate[0], labels[0] * interpolation_mag, labels[0])
    # wx2 = tf.where(labels[0] < label_interpolate[0], label_interpolate[0] * (1 - interpolation_mag), label_interpolate[0])
    # mixed_label = tf.expand_dims(tf.clip_by_value(wx1 + wx2, 0, 1), axis=0)
    mixed_label = tf.expand_dims(labels[0] * interpolation_mag + label_interpolate[0] * (1 - interpolation_mag), axis=0)
    ...
    # 8. concat the mapped vectors
    fake_dlatents_out = tf.concat([dlatents_replaced, dlabel], axis=-1)
