...
def run(dataset, data_dir, result_dir, config_id, num_gpus, total_kimg, gamma, mirror_augment, metrics):
    train     = EasyDict(run_func_name='training.training_loop_mirror_randomize_labels_bathtub_refactor.training_loop') # Options for training loop.
    G         = EasyDict(func_name='training.networks_stylegan2_label_mapping_concat.G_main')       # Options for generator network.
    D         = EasyDict(func_name='training.networks_stylegan2_label_mapping_concat.D_stylegan2')  # Options for discriminator network.
    G_opt     = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                  # Options for generator optimizer.
    D_opt     = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                  # Options for discriminator optimizer.
    G_loss    = EasyDict(func_name='training.loss_label_mapping_concat_interpolate_w_matching_rotation_real_labels_bathtub_refactor.G_logistic_ns_pathreg_interpolate')      # Options for generator loss.
    D_loss    = EasyDict(func_name='training.loss_label_mapping_concat_interpolate_w_matching_rotation_real_labels_bathtub_refactor.D_logistic_r1')              # Options for discriminator loss.
    ...
    desc += '-%dgpu' % num_gpus
    desc += '-label-mapping-concat-interpolate-matching-rotation-real-labels'
